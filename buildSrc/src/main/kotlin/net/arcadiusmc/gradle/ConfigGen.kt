...
const val NEW_DEPENDENCY_FORMAT = true

internal fun createPluginYml(it: Task) {
  val configYml = it.project.extensions.findByType(PaperConfig::class.java) ?: return
  validateConfig(configYml, it)

  val builder = StringBuilder()
      .append("# Auto-Generated by $SERVER_NAME Gradle plugin\n")
...
fun resolveProjectDependency(name: String, importing: Project): String? {
  ...
  val extension = project.extensions.findByType(ArcadiusExtension::class.java) ?: return project.name

  if (extension.skipDependency) {
    return null
  }

  val implProj = extension.implementedBy;
...
fun getProjectPluginName(project: Project): String {
  val paperYml = project.extensions.findByType(PaperConfig::class.java)
...
fun getVersion(project: Project): String {
  val version = Objects.toString(project.version)

  if (version.isBlank() || version == "unspecified" || version == "null") {
    // IDK man, what the hell do you return in this case
...
fun validateConfig(config: PaperConfig, it: Task) {
  if (config.main == null) {
