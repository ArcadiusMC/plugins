...
public class PieceGenerator {
  ...
  public StepResult generate() {
    ...
    while (!potentials.isEmpty()) {
      ...
      WeightedIterator<RoomType> it = potentials.iterator(config.getRandom());
      ...
      while (it.hasNext()) {
        ...
        Iterator<GateData> gIt = gates.iterator();
        ...
        while (gIt.hasNext()) {
          ...
          if (this.type == SectionType.ROOM) {
            ...
            if ((depth <= config.getMinDepth()
              ...
              int keepOpen = exits.size() == 1
                  ? 1
                  : rand.nextInt(1, Math.min(exits.size(), type.getMaxExits(config)));
