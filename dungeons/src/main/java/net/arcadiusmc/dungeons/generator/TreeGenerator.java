package net.arcadiusmc.dungeons.generator;

import static net.arcadiusmc.dungeons.generator.StepResult.FAILED;
import static net.arcadiusmc.dungeons.generator.StepResult.MAX_DEPTH;
import static net.arcadiusmc.dungeons.generator.StepResult.MAX_SECTION_DEPTH;
import static net.arcadiusmc.dungeons.generator.StepResult.SUCCESS;
...
import net.arcadiusmc.dungeons.gate.Gates;
...
public class TreeGenerator {
  ...
  private LevelGenResult compileResult() {
    ...
    IntSummaryStatistics stats = walker.endGates
        .stream()
        .mapToInt(DungeonPiece::getDepth)
        .summaryStatistics();

    LevelGenResult genResult = new LevelGenResult(
        level,
  ...
  public boolean isValidPlacement(DungeonPiece piece) {
    Bounds3i bb = piece.getBounds();
    Set<DungeonPiece> intersecting = level.getIntersecting(bb);

    if (intersecting.isEmpty()) {
      return true;
    }

    for (DungeonPiece p : intersecting) {
      Vector3i intersection = p.getBounds().intersection(bb).size();
  ...
  public void step() {
    ...
    GatePiece gate = section.getOrigin();
    StepResult result = section.generate();
    ...
    switch (result.getResultCode()) {
      ...
      case MAX_DEPTH -> {
        ...
        if (section.getSectionDepth() < section.getData().getOptimalDepth()
          ...
          PieceGenerator root = section.sectionRoot();
  ...
  PieceGenerator switchGeneratorType(
    ...
    SectionType type = existing.getType().next();
