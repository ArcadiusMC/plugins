...
import static net.arcadiusmc.dungeons.gate.GateData.TAG_CORRECT;
    ...
    return getFunctions()
        ...
        .map(info -> {
          ...
          if (info.getTag() != null && !info.getTag().isEmpty()) {
            ...
            applyCorrection = !tag.contains(TAG_CORRECT) || tag.getBoolean(TAG_CORRECT);
            stairs = !tag.contains(TAG_STAIR) || tag.getBoolean(TAG_STAIR);
          }
