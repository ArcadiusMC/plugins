...
import static net.arcadiusmc.cosmetics.travel.TravelUtil.spawnInCircle;
...
public class SmokeTravelEffect implements TravelEffect {
  ...
  public void onHulkStart(User user, Location loc) {
    // Cone shaped smoke
    spawnInCircle(loc, 1.3, 0.1, (short) 5, Particle.LARGE_SMOKE, 2);  // Inner circle
    spawnInCircle(loc, 0.8, 0.25, (short) 10, Particle.LARGE_SMOKE, 4);
    spawnInCircle(loc, 0.6, 0.5, (short) 10, Particle.LARGE_SMOKE, 4);
    spawnInCircle(loc, 0.3, 1, (short) 20, Particle.LARGE_SMOKE, 2);
    spawnInCircle(loc, 0.1, 1.5, (short) 20, Particle.LARGE_SMOKE, 1); // Outer circle

    // Extra explosion for good measure :D
    loc.getWorld().spawnParticle(Particle.EXPLOSION, loc.add(0, 0.1, 0), 1, 0, 0, 0, 0, null, true);
  }
