...
class PunishPage extends MenuPage {
  ...
  protected void createMenu(MenuBuilder builder) {
    ...
    for (var type : PunishType.values()) {
          ...
          MenuNode.builder()
              .setItem((user, context) -> {
                ...
                if (!user.hasPermission(type.getPermission())) {
                  item.addLore(GMessages.noPunishmentPermission(user, type));
                }
              ...
              .setRunnable((user, context, click) -> {
                ...
                PunishCommands.ensureCanPunish(user.getCommandSource(), target, type);
                ...
                if (type == PunishType.JAIL) {
                  ...
                  PunishBuilder p = new PunishBuilder(entry, PunishType.KICK);
                  p.punish(target.getCommandSource());
  ...
  static class JailSelectorPage extends ListPage<JailCell> {
    ...
    protected List<JailCell> getList(User user, Context context) {
      return Lists.newArrayList(Punishments.getJails().getCells());
    }
